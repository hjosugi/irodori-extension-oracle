...
初期のソーススナップショットは、デスクトップアプリの `db/oracle.rs` からコピーされました。

ソースSHA-256: `fa986acea3255b1bafb25edbe7f0d065c96fe211fc2ee20d5da8e4e832cbea98`。


このディレクトリは `irodori.oracle` のマイグレーションステージングエリアです。アクティブなネイティブのエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` にあり、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジン固有の接続／クエリ／メタデータコードは、コネクタのランタイム契約がデスクトップアプリに組み込まれるにつれて、これらのスナップショットからその動作モジュールに移動する必要があります。

## マイグレーションスナップショット

| 種類 | ソース | 目的地 | SHA-256 |
|---|---|---|---|
| `desktop-db-adapter` | `apps/desktop/src-tauri/src/db/oracle.rs` | `native/source/irodori-table/apps/desktop/src-tauri/src/db/oracle.rs` | `fa986acea3255b1bafb25edbe7f0d065c96fe211fc2ee20d5da8e4e832cbea98` |
